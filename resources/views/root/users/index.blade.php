...
@section('content')
            ...
            <div class="m-form m-form--label-align-right m--margin-top-20 m--margin-bottom-30">
                <div class="row align-items-center">
                    ...
                    <div class="col-xl-4 order-1 order-xl-2 m--align-right">
                        <a href="{{ route(user_env().'.users.create') }}" class="btn btn-primary m-btn m-btn--custom m-btn--icon m-btn--air m-btn--pill">
                            <span><i class="la la-plus"></i><span>New distributor</span></span>
                        </a>
    ...
    @component(user_env().'.components.modal')
        ...
        <!-- Toggle Form -->
        <form method="POST" action="" id="form-user-toggle" style="display: none;">
            @method('PATCH')
            @csrf
        </form>
    @endcomponent
...
@section('scripts')
    <script>
                ...
                var datatable = $('#table').mDatatable({
                    data: {
                        ...
                        source: {
                            read: {
                                method: 'GET',
                                url: '{!! route(user_env().'.users.datatables.index') !!}',
                                map: function(raw) {
                                    var dataSet = raw;
                    ...
                    columns: [
                        ...
                        },
                        {
                            ...
                            sortable: false,
                            overflow: 'visible',
                            template: function (user, index, datatable) {
                                var dropup = (datatable.getPageSize() - index) <= 4 ? 'dropup' : '';
                                var toggle = user.active ? 'off' : 'on';
                                ...
                                return '\
                                    <div class="dropdown ' + dropup + '">\
                                        ...
                                        <div class="dropdown-menu dropdown-menu-right">\
                                            <a href="'+ user.DT_RowData.image_route +'" class="dropdown-item">\
                                                <i class="la la-image"></i> Images\
                                            </a>\
                                            <a href="javascript:void(0);" class="dropdown-item user-toggle"\
                                                data-toggle="modal" \
                                                data-target="#confirm-user-toggle" \
                                                data-form="#form-user-toggle" \
                                                data-action="' + user.DT_RowData.toggle_route + '" \
                                                data-user-name="' + user.name + '"> \
                                                <i class="la la-toggle-'+ toggle +'"></i> Toggle-'+ toggle +'\
                                            </a>\
                                        </div>\
                                    </div>\
                                    <a href="'+ user.DT_RowData.edit_route +'" class="m-portlet__nav-link btn m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill" title="Edit details">\
                        ...
                        },                        
                        {
                            ...
                            template: function(user) {
                                ...
                                return moment(user.birthdate).format('MMMM Do, YYYY');
                            },
                        },
                        {
                            field: 'first_name',
                        ...
                        },
                        {
                            ...
                            template: function(user) {
                                if (user.creator != null) {
                                    var name = user.creator.name;
                                    var image = user.creator.file_path;
                        ...
                        },
                        {
                            ...
                            template: function (user) {
                                ...
                                return moment(user.created_at).format('MMMM Do, YYYY @ hh:mm:ss A');
                            },
                        },
                        {
                            field: 'updater',
                            ...
                            template: function(user) {
                                if (user.updater != null) {
                                    var name = user.updater.name;
                                    var image = user.updater.file_path;
                        ...
                        },
                        {
                            ...
                            template: function(user) {
                                ...
                                return moment(user.updated_at).format('MMMM Do, YYYY @ hh:mm:ss A');
                            },
                        },
                    ],
                });
        ...
        $(document).ready(function(e) {
            ...
            // toggle
            $('#table').on('click', '.user-toggle', function(e) {
                e.preventDefault();
