...
@section('content')
    <div class="m-portlet">
        ...
        <form method="POST" action="{{ route(user_env().'.products.update', $product) }}" id="form_update_product" class="m-form m-form--fit m-form--label-align-right m-form--group-seperator-dashed m-form--state">
            @method('PATCH')
            @csrf
	            ...
	            <div class="form-group m-form__group row {{ $errors->has('description') ? 'has-danger' : '' }}">
	                ...
	                <div class="col-lg-6">
	                    <textarea name="description" id="description" class="summernote {{ $errors->has('description') ?
	                    	' form-control-danger' : '' }}">{!! old('description') ?? $product->description  !!}
	                    </textarea>
                ...
                <div class="form-group m-form__group row {{ $errors->has('featured') ? 'has-danger' : '' }}">
                    ...
                    <div class="col-lg-6">
                        <span id="featured_wrapper" class="m-switch m-switch--lg m-switch--icon 
                            {{ $product->featured ? 'm-switch--success' : ' m-switch--danger' }}">
                            <label>
                                <input type="checkbox" name="featured" id="featured"
                                    {{ $product->featured ? 'checked="checked"' : '' }}>
                                <span></span>
                            </label>
                        </span>
    ...
    <script>
        var data = function () {
            ...
            var validator = function () {
            	var form = $("form[id=form_update_product]");
