        ...
        <div class="m-portlet__head">
            <div class="m-portlet__head-caption">
                <div class="m-portlet__head-title">
                    ...
                    <h3 class="m-portlet__head-text">Edit episode</h3>
                </div>
            </div>
        </div>

        <form method="POST" action="{{ route(user_env('episodes.update'), $episode) }}" id="form_update_episode" class="m-form m-form--fit m-form--label-align-right m-form--group-seperator-dashed m-form--state">
            @method('PATCH')
            @csrf
                    ...
                    <div class="col-lg-6">
                        <select name="tutorial" id="tutorial" class="form-control m-bootstrap-select">
                            ...
                            @foreach($tutorials as $tutorial)
                                <option value="{{ $tutorial->id }}" {{ old('tutorial') ?? $episode->tutorial_id ==
                        ...
                        <span class="m-form__help">The tutorial this episode belongs.</span>
                    </div>
                </div>
                <!--/. Tutorial -->
                    ...
                    <div class="col-lg-6">
                        <input type="text" name="number" id="number" class="form-control m-input
                            {{ $errors->has('number') ? 'form-control-danger' : '' }}"
                                placeholder="Please enter an episode number" value="{{ old('number') ?? $episode->number }}">
	            ...
	            <div class="form-group m-form__group row {{ $errors->has('body') ? 'has-danger' : '' }}">
	                ...
	                <div class="col-lg-6">
	                    <textarea name="body" id="body" class="summernote {{ $errors->has('body') ?
	                    	' form-control-danger' : '' }}">{!! old('body') ?? $episode->body !!}
	                    </textarea>
                    ...
                    <div class="col-lg-6">
                        <input type="text" name="video_url" id="video_url" class="form-control m-input
                            {{ $errors->has('video_url') ? 'form-control-danger' : '' }}"
                                placeholder="Please enter a video URL" value="{{ old('video_url') ?? $episode->video_url }}">
    ...
    <script>
        var data = function () {
            ...
            var validator = function () {
            	var form = $("form[id=form_update_episode]");
